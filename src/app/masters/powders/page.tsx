...
export default function PowdersPage() {
  ...
  const load = () => fetch("/api/powders").then(r => r.json()).then(setItems);
  useEffect(() => { load(); }, []);
  const add = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) return;
    setSaving(true);
    await fetch("/api/powders", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name }) });
    setName(""); setSaving(false); load();
  };
  const del = async (id: string) => { await fetch(`/api/powders/${id}`, { method: "DELETE" }); load(); };
  return (
    <div className="max-w-xl">
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Powders</h1>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        {items.length === 0 && <p className="px-4 py-8 text-center text-gray-400">No powders yet.</p>}
        {items.map((item, i) => (
